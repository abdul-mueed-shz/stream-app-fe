...
const navData = [
  ...
  { link: '/upcoming-broadcast', label: 'Upcoming Broadcast' },
  { link: '/previous-broadcast', label: 'Previous Broadcast' },
];
...
const Header = () => {
  ...
  return (
    <header className={header}>
      <div className={logo}>
        <Image src={images.logo_dark} alt='logo' height={35} width={35} />
          ...
          {navData.map(({ label, link }, i) => {
            return (
              <motion.div
                ...
                className={currentPage === link ? activeMobileMenuLink : mobileMenuLink}
              >
                <Link
                  style={{
                    textDecoration: 'none',
                    color: currentPage === link ? 'white' : 'rgba(255, 255, 255, 0.6)',
                  }}
                  href={link}
                  onClick={() => toggleOpen()}
                >
